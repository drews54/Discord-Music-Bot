...
async def load(
    ctx: commands.Context, extension: str
...
@bot.command(hidden=True)
async def reload(
    ctx: commands.Context, extension: str = "music"
):  # pylint: disable=unused-argument
...
async def about(ctx: commands.Context):
    ...
    await ctx.send(
        boxed_string(
            _("GitHub source code")
            + ": https://github.com/duha54rus/Discord-Music-Bot"
            + "\n"
            + _("Docker Hub repository")
            + ": https://hub.docker.com/r/drews54/discord-music-bot"
        )
    )
    ...
    if isinstance(error, commands.CommandNotFound):
        await ctx.send(
            ...
            **{}** command not found.
            Use {}help
            """
            ).format(ctx.message.content, bot.command_prefix)
        )
        ...
        await ctx.send(
            ...
            """
            ).format(error.param, bot.command_prefix, ctx.command)
        )
